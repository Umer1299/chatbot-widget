(function () {
  ...
  var STYLE_ID = "chatbot-widget-exact-chatflow-layout-v6";
  var BODY_STYLE_ID = "chatflow-proactive-body-style";
  var BODY_BOX_ID = "chatflow-proactive-body-messages";
  var FONT_LINK_ID = "chatflow-inter-font-link";
  var MAX_Z_INDEX = 2147483647;
  var SCAN_INTERVAL_MS = 80;
  var MAX_SCAN_ATTEMPTS = 400;
  var STATE_KEY = "__chatflowProactiveMessages";
  ...
  function getIconBgColor() {
    ...
    return normalizeCssColor(script.getAttribute("data-icon-bg-color") || script.getAttribute("data-launcher-bg-color") || script.getAttribute("data-button-bg-color") || "");
  }
  ...
  function getScriptProactiveMessages() {
    ...
    var attrs = ["data-proactive-messages", "data-proactive-message", "data-launcher-messages", "data-launcher-message", "data-welcome-messages", "data-welcome-message", "data-teaser-messages", "data-teaser-message"];
  ...
  function extractProactiveMessages(data) {
    ...
    var unique = [];
    for (var m = 0; m < messages.length; m += 1) if (unique.indexOf(messages[m]) === -1) unique.push(messages[m]);
  ...
  function storeProactiveMessagesFromConfig(data) {
    ...
    var position = response && response.position === "left" ? "left" : "right";
    window[STATE_KEY] = { messages: messages, position: position, updatedAt: Date.now() };
    renderBodyProactiveMessages();
  }

  function getStoredState() {
    var state = window[STATE_KEY] || {};
    if (!Array.isArray(state.messages) || !state.messages.length) state.messages = getScriptProactiveMessages().slice(0, 2);
    if (!state.position) state.position = "right";
    return state;
  }
  ...
  function isChatOpen() {
    var widgetRoot = findWidgetRoot();
    return widgetRoot && widgetRoot.getAttribute("data-open") === "true";
  }

  function installBodyProactiveStyle() {
    if (!document.head || document.getElementById(BODY_STYLE_ID)) return;
    var style = document.createElement("style");
    style.id = BODY_STYLE_ID;
    style.textContent = [
      "#" + BODY_BOX_ID + "{position:fixed!important;right:20px!important;bottom:116px!important;display:none!important;flex-direction:column!important;gap:10px!important;align-items:flex-end!important;max-width:min(360px,calc(100vw - 40px))!important;margin:0!important;padding:0!important;pointer-events:auto!important;z-index:" + MAX_Z_INDEX + "!important;font-family:Inter,Arial,sans-serif!important;}",
      "#" + BODY_BOX_ID + "[data-position='left']{right:auto!important;left:20px!important;align-items:flex-start!important;}",
      "#" + BODY_BOX_ID + " .chatflow-proactive-body-message{box-sizing:border-box!important;width:max-content!important;max-width:min(360px,calc(100vw - 40px))!important;background:#fff!important;color:#111827!important;border:1px solid rgba(17,24,39,.12)!important;border-radius:10px!important;box-shadow:0 8px 24px rgba(15,23,42,.10)!important;padding:13px 18px!important;font-size:14px!important;line-height:1.35!important;font-weight:400!important;letter-spacing:0!important;text-align:left!important;white-space:normal!important;cursor:pointer!important;}",
      "#" + BODY_BOX_ID + " .chatflow-proactive-body-message:hover{box-shadow:0 10px 28px rgba(15,23,42,.14)!important;}",
      "@media(max-width:767px){#" + BODY_BOX_ID + "{right:16px!important;bottom:calc(106px + env(safe-area-inset-bottom))!important;max-width:calc(100vw - 32px)!important;}#" + BODY_BOX_ID + "[data-position='left']{right:auto!important;left:16px!important;}#" + BODY_BOX_ID + " .chatflow-proactive-body-message{max-width:calc(100vw - 32px)!important;}}"
    ].join("\n");
    document.head.appendChild(style);
  }

  function renderBodyProactiveMessages() {
    if (!document.body) return;
    installBodyProactiveStyle();
    var state = getStoredState();
    var messages = state.messages || [];
    var box = document.getElementById(BODY_BOX_ID);
    if (!box) {
    ...
    for (var i = 0; i < messages.length; i += 1) {
      ...
      bubble.className = "chatflow-proactive-body-message";
    ...
    window.fetch = function (input, init) {
      ...
      return nativeFetch(input, init).then(function (response) {
        ...
        return response.clone().json().then(function (data) {
          ...
          window.setTimeout(renderBodyProactiveMessages, 0);
          window.setTimeout(renderBodyProactiveMessages, 120);
          window.setTimeout(renderBodyProactiveMessages, 500);
  ...
  function applyExactStyles(root) {
    ...
    var launcherBg = iconBg || "var(--chatbot-primary,#1450d8)";
    style.textContent = [
      ":host{z-index:" + MAX_Z_INDEX + "!important;--chatbot-launcher-bg:" + launcherBg + "}",
      ...
      ".chat-header .title{font-size:18px!important;font-weight:800!important;line-height:1.2!important;color:#fff!important;flex:1!important}",
      ...
      ".message.bot .bot-cta{display:block!important;align-self:flex-start!important;margin:8px 0 0 0!important;max-width:82%!important;white-space:normal!important;text-align:center!important}",
      ...
      ".composer textarea{box-sizing:border-box!important;width:100%!important;padding:0!important;border:0!important;background:transparent!important;outline:0!important;resize:none!important;line-height:1.35!important;display:block!important;box-shadow:none!important;font:inherit!important;color:#111827!important}",
      ...
      ".branding{display:none!important;text-align:center!important;padding:6px 12px 8px!important;font-size:10px!important;line-height:1.05!important;border-top:0!important;background:#fff!important;color:#9ca3af!important;font-weight:400!important}",
      ".branding[data-visible='true']{display:block!important}.branding a{color:#9ca3af!important;text-decoration:none!important;font-size:10px!important}.branding a .chatflow-brand-text{color:#0949c7!important;font-weight:800!important;font-size:10px!important}",
  ...
  function resizeComposerInput(root) {
    ...
    var defaultHeight = window.matchMedia && window.matchMedia("(max-width:767px)").matches ? 22 : 24;
    var maxHeight = window.matchMedia && window.matchMedia("(max-width:767px)").matches ? 112 : 118;
    ...
    var isMultiline = (textarea.value || "").indexOf("\n") !== -1 || measuredHeight > defaultHeight + 3;
  ...
  function styleBrandingText(root) {
    ...
    span.className = "chatflow-brand-text";
    span.textContent = text.slice(match.index, match.index + match[0].length);
    link.appendChild(span);
    link.appendChild(document.createTextNode(text.slice(match.index + match[0].length)));
  }
  ...
  function attachExactLayoutBehavior(root) {
    ...
    root.addEventListener("click", function () { window.setTimeout(schedule, 0); window.setTimeout(renderBodyProactiveMessages, 80); }, true);
